...
function extract_multi_language(
  str,
  options = {
    remove_digits: true,
  ...
  const return_common = options.return_common
  const results = []
  languages.forEach(language => {
    const newOptions = options
    newOptions.language = language
    const result = extract(str,newOptions)
    results.push(result)
  });
  
  if(return_common)
    return findCommonElements(results)
  else
    return flattenAndRemoveDuplicates(results)
}

// Function to find common elements between arrays
function findCommonElements(results) {
  if (results.length === 0) return [];
  
  const commonElements = results.reduce((acc, currentArray) => {
    return acc.filter(element => currentArray.includes(element));
  });

  return [...new Set(commonElements)]; // Remove duplicates
}

function flattenAndRemoveDuplicates(results) {
  const mergedArray = [].concat(...results); // Flatten the arrays
  const uniqueArray = Array.from(new Set(mergedArray)); // Remove duplicates
